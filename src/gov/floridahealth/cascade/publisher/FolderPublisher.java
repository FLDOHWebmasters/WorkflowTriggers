...
public class FolderPublisher extends Publisher {
	...
	public boolean process() throws TriggerProviderException {
		...
		try {
			...
			String err = String.format("Could not get commonWorkflow for workflow %s",
					new Object[] { this.workflow.getName() });
			LOG.fatal(err, t);
		...
		if (siteId != null) {
			...
			/*
			 * No, I do not know why it needs the ID of the folder and the Folder object of
			 * what it's publishing. What I do know is that without both, it throws up.
			 * C'est la vie.
			 */
	...
	/**
	 * Attempts to retrieve the folder object (which is itself a
	 * FolderContainedEntity)
	 * 
	...
	private FolderContainedEntity TryGetFolder(String siteId, String path) {
		...
		try {
			...
			folder = this.serviceProvider.getLocatorService().locateFolderContainedEntity(path, EntityTypes.TYPE_FOLDER,
					siteId);
		} catch (Exception e) {
	...
	/**
	 * Obtains the FolderContainedEntity object represenatation of the asset, then
	 * pulls the Site ID from that.
	 * 
